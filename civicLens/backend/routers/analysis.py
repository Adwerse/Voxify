...
def _infer_conflicts_from_responses(response_texts: list[str], themes: list[dict[str, Any]]) -> list[dict[str, Any]]:
	lower_text = "\n".join(response_texts).lower()
	theme_hint = str(themes[0].get("label", "student priorities")) if themes else "student priorities"
	conflicts: list[dict[str, Any]] = []

	rules = [
		(
			("fast", "quick", "speed", "sooner"),
			("detailed", "quality", "thorough", "depth"),
			"Delivery speed vs depth",
			"Some students prefer faster delivery and shorter turnaround.",
			"Others prioritise deeper quality, even if timelines are longer.",
		),
		(
			("strict", "enforce", "mandatory"),
			("flexible", "choice", "optional", "adapt"),
			"Consistency vs flexibility",
			"Part of the cohort asks for stricter, consistent rules.",
			"Another segment asks for more flexible, context-aware options.",
		),
		(
			("security", "guard", "monitor", "visible staff"),
			("intimidating", "anxious", "overpolic", "comfortable"),
			"Safety presence trade-off",
			"Visible support staff can increase perceived safety.",
			"High visibility can also feel intimidating for some students.",
		),
	]

	for positive_markers, negative_markers, topic, pos_text, neg_text in rules:
		if any(marker in lower_text for marker in positive_markers) and any(marker in lower_text for marker in negative_markers):
			conflicts.append(
				{
					"topic": topic,
					"position_a": pos_text,
	...
	if not conflicts and len(response_texts) >= 6:
		conflicts.append(
			{
				"topic": f"Prioritisation within {theme_hint}",
				"position_a": "A larger group asks for immediate practical fixes.",
				"position_b": "Another group prefers longer-term structural improvements.",
				"split_estimate": "60-40",
			}
		)
...
def _build_analytics_payload(db: Session, poll: Poll, analysis: Analysis, responses: list[Response]) -> dict[str, Any]:
	...
	top_theme_label = str(themes[0].get("label", "student priorities")) if themes else "student priorities"
	representation_health = str(missing_voices.get("representation_health", "unknown"))
	if representation_health == "poor":
		impact_summary = f"Policy impact around {top_theme_label} is high, but decisions risk bias unless underrepresented cohorts are engaged."
	elif representation_health == "moderate":
		impact_summary = f"{top_theme_label} is a clear signal; targeted outreach could improve confidence before implementation."
	else:
		impact_summary = f"{top_theme_label} shows a stable signal with relatively balanced representation across respondent groups."
...
def _build_student_report(analytics: dict[str, Any]) -> dict[str, Any]:
	...
	for item in themes[:5]:
		...
		theme_items.append(
			{
				"theme": label,
				"key_insight": f"{response_count} students raised this, making it one of the strongest current signals.",
				"quote": quote,
			}
		)
	...
	theme_labels = [entry["theme"] for entry in theme_items[:2]]
	lead_message = " and ".join(theme_labels) if theme_labels else "daily student experience"

	return {
		"poll_id": analytics.get("poll_id"),
		"generated_at": analytics.get("generated_at"),
		"what_students_said": f"Students are most vocal about {lead_message}, with practical requests that can be acted on quickly.",
...
def _build_institutional_report(analytics: dict[str, Any]) -> dict[str, Any]:
	...
	considerations = [
		...
		"Define a short implementation horizon (2-6 weeks) and publish measurable success indicators.",
...
def analyse_poll(poll_id: str, db: Session = Depends(get_db)) -> JSONResponse:
	...
	try:
		...
		themes_result = _call_openai_json(
			client,
			openai_model,
			"You are a civic engagement analyst. Return only valid JSON and ground every theme in the provided responses.",
			(
				...
				"  ]\n"
				"}\n"
				"Max 6 themes. Order by response_count descending. Quotes must be copied or lightly trimmed from responses."
			),
		)
	...
	try:
		...
		call_2 = _call_openai_json(
			client,
			openai_model,
			"You are a civic engagement analyst. Detect sentiment and real contradictions, not superficial disagreement.",
			(
				...
				"  ]\n"
				"}\n"
				"Only include genuine conflicts where both sides are meaningfully represented in responses."
			),
		)
		sentiment_result = {
	...
	if not conflicts_result:
		...
		if isinstance(themes_array, list):
			conflicts_result = _infer_conflicts_from_responses(response_texts, themes_array)
	...
	try:
		...
		briefing_result = _call_openai_json(
			...
			(
				...
				"}\n"
				"Tone: formal but accessible. No jargon. Keep recommendations specific and non-prescriptive."
			),
		)
	except Exception as exc:
